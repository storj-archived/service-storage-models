...
describe('Storage/models/Credit', function() {
  ...
  describe('#create', function() {
    ...
    it('should fail if paid_amount is greater than invoiced_amount',
      function(done) {
        var newCredit = new Credit({
          user: 'user@domain.tld',
          type: CREDIT_TYPES.MANUAL,
        ...
        newCredit.save(function(err) {
          ...
          expect(err.message).to.equal(
            `Cannot save credit: paid_amount cannot be greater than invoiced_amount` // jshint ignore:line
          );
          done();
        });
    });

    it('should set paid = true if invoiced_amount is > than 0 && = paid_amount',
      function(done) {
        var newCredit = new Credit({
          user: 'user@domain.tld',
          type: CREDIT_TYPES.MANUAL,
    ...
    it('should fail if paid is true and paid_amount !== invoiced_amount',
      function(done) {
        var newCredit = new Credit({
          user: 'user@domain.tld',
          type: CREDIT_TYPES.MANUAL,
        ...
        newCredit.save(function(err) {
          expect(err).to.be.instanceOf(Error);
          expect(err.message).to.equal(
            `Cannot save credit: paid cannot be true if paid_amount does not equal invoiced_amount` // jshint ignore:line
          );
          done();
        });
    });

    it('should fail if paid = true and paid_amount and/or invoiced_amount is 0',
      function(done) {
        var newCredit = new Credit({
          user: 'user@domain.tld',
          type: CREDIT_TYPES.MANUAL,
    ...
    it('should have "none" promo_code if promo_amount is equal to 0',
      function(done) {
        var newCredit = new Credit({
          user: 'user@domain.tld',
          type: CREDIT_TYPES.MANUAL,
    ...
    it('should have other promo_code if promo_amount is greater than 0',
      function(done) {
        var newCredit = new Credit({
          user: 'user@domain.tld',
          type: CREDIT_TYPES.MANUAL,
