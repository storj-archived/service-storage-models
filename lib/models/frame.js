...
Frame.methods.addShard = function(pointer, callback) {
  ...
  function shouldAddShard() {
    // Do not allow upload of files with multiple shards smaller than 2mb
    if (self.shards.length > 1) {
      let i = 0;
      for (i in self.shards) {
        // The last shard is usually smaller than the rest and that's okay
        if (self.shards[i].size < 2097152 &&
            i !== self.shards.length - 1 &&
            !self.shards[i].parity) {
          return false;
        }
      }
    }
  ...
  if (!shouldAddShard()) {
    return callback(new Error('Shard size is too small'));
  }
