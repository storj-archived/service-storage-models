...
/**
 * Checks if the rates of reports resolving to unknown exceed thresholds
 * @param {Number} threshold - A number between 0 and 1
 */
UserSchema.methods.exceedsUnknownReportsThreshold = function(threshold) {
  const totalRate = this.reports ? this.reports.totalRate : undefined;
  const unknownRate = this.reports ? this.reports.unknownRate : undefined;
  if (!unknownRate || !totalRate) {
    return false;
  }
  if (totalRate / unknownRate > threshold) {
    return true;
  }
  return false;
};
...
/**
 * Will update reporting statistics for a user
 *
 * Rates are stored as an estimated moving average of the time between
 * reports. The lower the number the faster and more frequent the reports are
 * on average.
 *
...
UserSchema.methods.updateUnknownReports = function(unknown,
  ...
  assert(timestamp instanceof Date, 'timestamp is expected to be a Date');

  /**
   * Calculates estimated moving average
   * @param {Number} a - Previous average
   * @param {Number} b - Value to update average
   */
  function ema(a, b) {
    const p = 5000; // The number of reports in decay
    const k = 2 / (p + 1);
    return b * k + a * (1 - k);
  }
  ...
  if (this.reports && this.reports.totalRateTimestamp) {
    totalPrevTimestamp = this.reports.totalRateTimestamp;
  }

  // Check that this update hasn't already been applied
  if (totalPrevTimestamp > timestamp) {
    // TODO give error?
    return callback();
  }
  ...
  let update = {
    $set: {
      'reports.totalRate': totalRate,
      'reports.totalRateTimestamp': timestamp
    }
  };

  // If the report has an unknown status also update the rate
  // for unknown reports
  if (unknown) {
    let unknownPrevRate = 0;
    let unknownPrevTimestamp = 0;
    if (this.reports && this.reports.unknownRate) {
      unknownPrevRate = this.reports.unknownRate;
    }
    if (this.reports && this.reports.unknownRateTimestamp) {
      unknownPrevTimestamp = this.reports.unknownRateTimestamp;
    }

    if (!unknownPrevTimestamp) {
      unknownPrevTimestamp = timestamp - defaultDelta;
    }
    const unknownDelta = timestamp - unknownPrevTimestamp;
    const unknownBandwidth = bytes / unknownDelta;
    const unknownRate = ema(unknownPrevRate, unknownBandwidth);

    update.$set['reports.unknownRate'] = unknownRate;
    update.$set['reports.unknownRateTimestamp'] = timestamp;
  } else {
    update.$set['reports.unknownRateTimestamp'] = timestamp;
  }

  // Make sure to only update if not already updated by using
  // the totalRateTimestamp that is updated every time.
  this.collection.update({
    '_id': this._id,
    $or: [
      { 'reports.totalRateTimestamp': { $lt: timestamp } },
      { 'reports.totalRateTimestamp': { $exists: false } },
    ]
  }, update, (err) => {
    if (err) {
      return callback(err);
