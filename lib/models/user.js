...
User.methods.generateDebitsForStorage = function(beginTimestamp, endTimestamp) {
  const amountOwedStorage = usage.amountOwedForStorage(
      this._id,
      beginTimestamp,
      endTimestamp,
      CENTS_PER_GB_HOUR
  );
  const amountOwedBandwidth = usage.amountOwedForBandwidth(
      this._id,
      beginTimestamp,
      endTimestamp,
      CENTS_PER_GB_TRANSFER
  );

  // create debit on user account for billed amount
  Debits.prototype.createDebit(this._id, DEBIT_TYPES.STORAGE, amountOwedStorage);
  Debits.prototype.createDebit(this._id, DEBIT_TYPES.BANDWIDTH, amountOwedBandwidth);
};
