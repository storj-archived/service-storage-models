 ...
 * @param {Function} callback
 */
User.methods.recordUploadBytes = function(bytes, callback) {
  let now = Date.now();
  let {lastHourStarted, lastDayStarted} = this.bytesUploaded;
 ...
 * @returns {Boolean}
 */
User.methods.isUploadRateLimited = function(hourlyBytes, dailyBytes) {
  return this.isFreeTier
    ? (this.bytesUploaded.hourlyBytes >= hourlyBytes) &&
      (this.bytesUploaded.dailyBytes >= dailyBytes)
    : false;
};
