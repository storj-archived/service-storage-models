...
PaymentProcessor.methods.defaultCard = function() {
  return this.adapter.defaultPaymentMethod(this);
};

PaymentProcessor.methods.billingDate = function() {
...
PaymentProcessor.methods.update = function(paymentProcessor) {
  return this.adapter.validate(paymentProcessor)
    .then((isValid) => {
      ...
      console.log('__parent _id: ', this.__parent._id);
      UserModel.find({
      ...
      }).then((user)=> console.log('User.Find: ', user));
