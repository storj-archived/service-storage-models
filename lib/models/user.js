...
if (typeof APP_PASSWORD_SALT === 'undefined') {
  // TODO: phase this out later...
  //throw new Error('no APP_PASSWORD_SALT specified');
  logger.warn('APP_PASSWORD_SALT specified: defaulting to old password hashing method');
}
...
UserSchema.statics.create = function(email, passwd, callback) {
  // TODO: phase this out
  const string = (typeof APP_PASSWORD_SALT === 'undefined') ?
    passwd : `${APP_PASSWORD_SALT}+${passwd}`;

  let User = this;
  let user = new User({
    _id: email,
    lastPasswordReset: moment.utc().valueOf(),
    hashpass: crypto
      .createHash('sha256')
      .update(string)
      .digest('hex')
...
UserSchema.statics.lookup = function(email, passwd, callback) {
  ...
  User.findOne({
    _id: email,
    hashpass: crypto.createHash('sha256').update(string).digest('hex')
  }, function(err, user) {
    if (err) {
      return callback(new errors.InternalError(err.message));
    }
    ...
    if (!user) {
      ...
      User.findOne({_id: email}, function(err, user) {
        logger.warn(`user with email: ${email} wasn't locaced using email and old password hashing method`);
        if (err) {
          return callback(new errors.InternalError(err.message));
        }

        const nonSaltedHashpass = crypto.createHash('sha256').update(passwd).digest('hex');
        if (user && !user.lastPasswordReset && user.hashpass === nonSaltedHashpass) {
          // TODO: send email to tell user to reset password cos security is getting better
          logger.warn(`user with email: ${email} is still using old password hashing method`);
          return callback(null, user);
        }
