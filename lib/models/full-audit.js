...
FullAudit.statics.scheduleFullAuditsFromShard = function(
  ...
  shardModel.find({hash: {$in: hashes}}).lean().exec((err, shards) => {
    let auditJobs = [];
    let auditDict  = {};
    ...
    let squish = (elem) =>  {
      ...
      elemKeys.forEach((key) => {
        if(key !== 'nodeID') {
          ...
          auditDict[elem.nodeID] = auditDict[elem.nodeID] || {};
        }
      });
    };
    ...
    for(let node in auditDict) {
      ...
      let job = {
          start: auditDict[node].contract.store_begin,
          end: auditDict[node].contract.store_end,
          farmer_id: auditDict[node].contract.farmer_id,
          data_hash: auditDict[node].contract.data_hash,
          root: auditData.root,
    ...
    return this.scheduleFullAudits(auditJobs, null, callback);
  });
};
