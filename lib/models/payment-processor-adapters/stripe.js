...
const stripeAdapter = {
  ...
  paymentMethods: function() {
    // TODO: better error handling (where used) in case structure isn't as expected
    try {
      return !!stripeProcessor.data && !!stripeProcessor.data.customer ?
        stripeProcessor.data.customer.sources.data : [];
    } catch (err) {
      console.error(
        'Error in service-storage-models/lib/models/payment-processor-adapters/stripe.js#paymentMethods ' +
  ...
  balanceDebits: function(customerId, debits) {
    // -- get invoices
    // -- do "stripe balance" calcualation
    // -- check for min amount
    return new Promise((resolve, reject) => {
      stripe.invoices.list({
        // NB: if a user accumulates more than 100 invoice items
        //     in a given invoice, we won't get them from stripe
        //     for the purposes of balance calculation between
        //     stripe invoices and storj debits.
        limit: 100,
        customer: customerId,
        date: {
          gte: startDate.unix(),
      ...
      }, (err, invoices) => err ? reject(err) : resolve(invoices));
    })
      .then((invoices) => {
        const invoiceTotal = (invoices.data[0].lines.data
          .reduce((result, invoice) => result + invoice.amount, 0.0) + 0.0);

        const debitTotal = (debits
          .reduce((result, debit) => result + debit.invoiced_amount, 0.0) + 0.0);

        const stripeBalance = debitTotal - invoiceTotal;
        ...
        return new Promise((resolve, reject) => {
          ...
          if (stripeBalance < 0.0) {
            ...
            stripe.invoiceItems.create({
              ...
              if (err) {
                console.error('Error in payment-processor-adaptersservice-storage-models/lib/models/payment-processor-adapters/stripe.js#balanceDebits');
                console.error('invoiceItem: %j', invoiceItem);
                console.error('debit: %j', debit);
                console.error(err);
