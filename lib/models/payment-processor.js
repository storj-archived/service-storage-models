...
const getBillingPeriodFor = function(startMoment, billingDate){
  const endOfBillingMonth = moment
    .utc([startMoment.year(), startMoment.month()])
    .add(1, 'month')
    .subtract(1, 'day')
    .date();
...
PaymentProcessorSchema.virtual('currentBillingPeriod').get(function() {
  const startMoment = moment.utc();
  return getBillingPeriodFor(startMoment, this.date.billingDate)
});

PaymentProcessorSchema.virtual('nextBillingPeriod').get(function() {
  const startMoment = moment.utc().add(1, 'month');
  return getBillingPeriodFor(startMoment, this.date.billingDate);
});
